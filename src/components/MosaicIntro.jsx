      ...
      for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
          ...
          tiles.current.push({
            ...
            targetRot: (Math.random() - 0.5) * 0.15,
            scale: 1,
          });
        }
      }
    ...
    function draw(ts) {
      ...
      if (localPhase === "build") {
        ...
        if (progress > 0.4) {
          ...
          ctx.fillText("JOIN THE MOSAIC", W / 2 + 4, H / 2 - 20 + 4);
          ctx.fillStyle = "#ffffff";
          ctx.fillText("JOIN THE MOSAIC", W / 2, H / 2 - 20);

          // Subtitle in handwriting
          ctx.font = `${Math.min(20, W * 0.028)}px 'Shadows Into Light Two', cursive`;
          ctx.fillStyle = "rgba(184,255,0,0.9)";
          ctx.fillText("RECRUITMENT 2026 ✦", W / 2, H / 2 + 36);
          ctx.restore();
        }
